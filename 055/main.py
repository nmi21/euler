...
# determine if something is palindrome
def is_palindrome(n):
    n = str(n)
    return n == n[::-1]
...
def add_reverse(n):
    # print(f"{n} + {int(str(n)[::-1])} == {n + int(str(n)[::-1])}")
...
def is_lychrel(n):
    ...
    i = 1
    n = add_reverse(n)
    while (not is_palindrome(n)) and (i < 51):
        n = add_reverse(n)
        i += 1
    # print(f"i == {i}")
    if i == 51:
...
limit = 10000
res = [x for x in list(range(1, limit)) if is_lychrel(x)]
